...
export async function signInWithGoogle(){ const provider=new GoogleAuthProvider(); return await signInWithPopup(auth, provider); }
export async function signOutUser(){ return await signOut(auth); }
export function watchAuth(cb){ onAuthStateChanged(auth, cb); }
export async function uploadTemplate(uid, file){ const r=ref(storage, `user_uploads/${uid}/lr-template.png`); await uploadBytes(r, file); return await getDownloadURL(r); }
